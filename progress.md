...
- Current focused coverage now includes:
  ...
  - Update Readiness operator checklist, audit verification, preflight, update candidate, and update-plan presentation state
  - Update Readiness snapshot detail, artifact, artifact-diff, active-plugin, and comparison presentation state
  - Update Readiness health baseline and health comparison presentation state
  ...
  - Update Readiness restore impact and run-link presentation state
...
- Main is now current through the merged Update Readiness validation detail-state cleanup for:
...
- Main is now current through the merged Update Readiness restore status-table cleanup for:
  - dry-run/stage/plan status badge and label state prepared by `UpdateReadinessStateBuilder`
  - dry-run/stage/plan check rows prepared before rendering
  - restore-plan item rows normalized before rendering
  - focused regression coverage for restore validation and plan-row state
- Main is now current through the merged Update Readiness execution result-state cleanup for:
  - restore execution and rollback result status badges prepared by `UpdateReadinessStateBuilder`
...
- Main is now current through the merged Update Readiness form/checkpoint presentation-state cleanup for:
...
- Main is now current through the merged Update Readiness status-section presentation-state cleanup for:
  - operator checklist badge, message, count label, and check rows prepared by `UpdateReadinessStateBuilder`
  - audit verification status and check rows prepared before rendering
  - preflight status and check rows prepared before rendering
  - update candidate and last update-plan target rows normalized before rendering
  - focused regression coverage for status-section rows, badges, labels, and empty defaults
- Main is now current through the merged Update Readiness snapshot detail presentation-state cleanup for:
  - snapshot basics and stored metadata rows prepared by `UpdateReadinessStateBuilder`
  - component manifest, rollback package, artifact diff, and active plugin rows prepared before rendering
  - snapshot comparison rows and plugin comparison lists prepared before rendering
  - focused regression coverage for snapshot detail rows, artifact rows, comparison rows, and empty defaults
- Main is now current through the merged Update Readiness health-baseline presentation-state cleanup for:
  - snapshot health baseline status state prepared by `UpdateReadinessStateBuilder`
  - health comparison rows normalized before rendering
  - focused regression coverage for baseline status, comparison counts, deltas, and empty defaults
- Main is now current through the merged Update Readiness restore-assessment presentation-state cleanup for:
  - restore readiness assessment status state prepared by `UpdateReadinessStateBuilder`
  - restore readiness assessment check rows normalized before rendering
  - focused regression coverage for assessment status, check rows, and empty defaults
- Main is now current through the merged Update Readiness activity/navigation presentation-state cleanup for:
...
- Main is now current through the merged Update Readiness form-defaults presentation-state cleanup for:
...
- Current branch extends the Update Readiness run-link presentation-state cleanup with:
  - restore impact summary rows and blocker messages normalized by `UpdateReadinessStateBuilder`
  - restore execution and rollback run URLs prepared before rendering
  - focused regression coverage for impact summary state, backup-root visibility, and execution/rollback run links
...
- Likely candidates:
  ...
  - any remaining raw target-link or health-status fallback formatting still embedded in the Update Readiness view
- Reason: controller-level, workspace-level, validation-row, restore validation/planning, execution/rollback result, journal row, checkpoint summary, form/resume, status-section, snapshot-detail, health-baseline, restore-assessment, activity/navigation, snapshot-list, form-default, and run-link state are now extracted, so the next highest-value work is reducing residual fallback rendering still concentrated in the view
...
- Current branch prepared for merge:
  - `feature/update-readiness-run-link-state`
- Next likely restart task after this branch merges:
  - start from `includes/admin/views/update-readiness.php`
  - extract the next residual table fallback, target-link, or health-status presentation-state seam into the Update Readiness state builder or a focused presenter
